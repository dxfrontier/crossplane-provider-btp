...
func TestToCreateApiPayload_Origin(t *testing.T) {
	tests := map[string]struct {
		reason     string
		origin     string
		wantOrigin *string
		...
		"EmptyOrigin": {
			reason:     "Empty origin should not set Origin field (backward compatible)",
			origin:     "",
			wantOrigin: nil,
		},
		"CustomIdpOrigin": {
			reason:     "Custom IDP origin should be passed to the API payload",
			origin:     "arrevqqkn-platform",
			wantOrigin: internal.Ptr("arrevqqkn-platform"),
		},
		"SapDefaultOrigin": {
			reason:     "SAP default origin should also be passed through",
			origin:     "sap.default",
			wantOrigin: internal.Ptr("sap.default"),
		},
	}
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			cr := NewSubaccount("test-sa", WithData(v1alpha1.SubaccountParameters{
				DisplayName:      "Test",
				Region:           "eu10",
				Subdomain:        "test-sub",
				SubaccountAdmins: []string{"admin@example.com"},
			}))
			payload := toCreateApiPayload(cr, tc.origin)
			if diff := cmp.Diff(tc.wantOrigin, payload.Origin); diff != "" {
				t.Errorf("\n%s\ntoCreateApiPayload() Origin: -want, +got:\n%s\n", tc.reason, diff)
			}
		})
	}
}
